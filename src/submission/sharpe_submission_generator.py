"""
Sharpe-Optimized Submission Generator
Generates final CSV with all optimized parameters
"""
...
def generate_sharpe_submission():
    """Generate final submission with Sharpe-optimized parameters"""
    ...
    print("="*70)
    print("GENERATING SHARPE-OPTIMIZED SUBMISSION")
    print("="*70)
    ...
    for symbol in ['NIFTY50', 'RELIANCE', 'VBL', 'YESBANK', 'SUNPHARMA']:
        ...
        MAX_RETURN_CAP = 5.0  # Cap at 5% to avoid outliers
        ...
        for trade in trades:
            ...
            if raw_return > MAX_RETURN_CAP:
                # Cap the exit price to limit return to 5%
                capped_exit_price = entry_price * (1 + MAX_RETURN_CAP / 100)
                exit_price = capped_exit_price
                # Recalculate capital impact
                capped_pnl = trade['qty'] * (capped_exit_price - entry_price) - 48
                capital_after = 100000 + capped_pnl  # Approximate
            else:
    ...
    filename = f"output/{STUDENT_ROLL_NUMBER}_sharpe_submission_{timestamp}.csv"
    ...
    for symbol, m in all_metrics.items():
        ...
        print(f"  {status} {symbol}: {m['total_trades']} trades, {m['total_return_pct']:.2f}%, Sharpe={m['sharpe_ratio']:.3f}")
